...
public class KinesisRecordProcessorTest {
  ...
  @Test
  public void shouldCheckpointShardEndWhenManualAcknowledgementsAreComplete() throws Exception {
    when(configuration.isAutoCommitEnabled()).thenReturn(false);
    objectUnderTest =
        new KinesisRecordProcessor(
            (event, acknowledgement) -> acknowledgement.ack(event),
    ...
    objectUnderTest.processRecords(sampleMessage(gson.toJson(new ProjectCreatedEvent())));
    objectUnderTest.shardEnded(ShardEndedInput.builder().checkpointer(checkpointer).build());

    verify(checkpointer).checkpoint();
  }

  @Test
  public void shouldFailShardEndWhenManualAcknowledgementsArePending()
