...
class KinesisRecordProcessor implements ShardRecordProcessor {
  ...
  public void shardEnded(ShardEndedInput shardEndedInput) {
    ...
    } else if (ackRecords.isEmpty()) {
      logger.atInfo().log("Reached shard end with no pending acknowledgements, checkpointing.");
      checkpoint(shardEndedInput.checkpointer());
    } else {
      int pendingRecords = ackRecords.size();
      logger.atSevere().log(
          "Reached shard end with %d pending acknowledgement(s); refusing completion"
              + " checkpoint in manual acknowledgement mode.",
          pendingRecords);
      throw new IllegalStateException(
          String.format(
              "Cannot complete shard end with %d pending acknowledgement(s)", pendingRecords));
    }
  }
